"""Program that tracks the face of the subject that appears on the camera for the verification process (turnstile simulation). 
...
def main():
	try:
		...
		print "Please Choose one of the two algorithms avaliable! (dlib, OpenFace or DeepFace)"
			...
			if detector.face_detection(frame) == 1:  #face detection
  				...
  				if detector.yaw_movement(detector.img2, detector.bb) is True: #Program continues if the person is looking towards the camera
  					...
  					if rep2 is not None:
  						...
  						print "Comparision between frames: {}".format(d)
  						start_face = time.time() #if there isn't a face image suitable for the comparision for more than 2 seconds, ss.send_warning()
  						flag_2sec = True
  						if d > verificator.threshold:   #It may not be the same person!
  							person_change = True
  							ss.send_warning()
  							start_text = time.time()		
  						if person_register is True and detector.blink_eyes(detector.img2, detector.bb) is True: #Face images of people that are looking towards the camera 
  																												#and with the eyes open are suitable for comparision with the database
  							cv2.rectangle(frame, (detector.cc[0], detector.cc[1]), (detector.cc[2], detector.cc[3]), (0,255,0), 3) #Draw Bounding Box in green	
							aux.send_descriptors_socket2(rep1, person_change)		
					rep2 = rep1
