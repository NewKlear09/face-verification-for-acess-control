...
int SocketServer::ReceiveSignal(int& number) {
  ...
  //printf("number22222 %s\n", buf );
  bytes_sent = recv(sock_fdesc_conn_, &buf, sizeof(buf), MSG_DONTWAIT);
  ...
  if((size_t)bytes_sent != sizeof(buf)) 
  {
    //printf("ERROR!: lenght failed\n");
    return 0;
  }
